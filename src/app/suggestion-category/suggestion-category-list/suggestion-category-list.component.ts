...
@Component({
  selector: 'app-suggestion-category-list',
  templateUrl: './suggestion-category-list.component.html',
  styleUrls: ['./suggestion-category-list.component.css']
})
export class SuggestionCategoryListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('suggestionCategoryPage') != null) {
      this.permission = JSON.parse(localStorage.getItem('suggestionCategoryPage'));
    }
  }
  ...
  create() {
    this.router.navigate(['suggestion-category-edit', ''], { skipLocationChange: true });
  }

  view(param) {
    this.router.navigate(['suggestion-category-edit', param], { skipLocationChange: true });
  }
  ...
  delete(param, idx) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        this.serviceProviderService.post('suggestion/category/delete', { code: param.code }).subscribe(data => {
  ...
  deleteAll() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        ...
        this.serviceProviderService.post('suggestion/category/delete', { code: model }).subscribe(data => {
          this.toastr.success('ลบรายการเรียบร้อย', 'แจ้งเตือนระบบ', { timeOut: 2000 });
          setTimeout(() => {
