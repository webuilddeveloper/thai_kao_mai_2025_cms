...
@Component({
  selector: 'app-policy-party-list',
  templateUrl: './policy-party-list.component.html',
  styleUrls: ['./policy-party-list.component.css']
})
export class PolicyPartyListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('policyPartyPage') != null) {
      let model: any = [];
      this.permission = JSON.parse(localStorage.getItem('policyPartyPage'));
  ...
  create() {
    this.router.navigate(['policy-party-edit', ''], { skipLocationChange: true });
  }

  view(param) {
    this.router.navigate(['policy-party-edit', param], { skipLocationChange: true });
  }
  ...
  delete(param, idx) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        this.serviceProviderService.post('policyParty/delete', { code: param.code }).subscribe(data => {
  ...
  reOrder() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        ...
        this.serviceProviderService.post('policyParty/reorder', { code: model }).subscribe(data => {
  ...
  reOrderAll() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        this.serviceProviderService.post('policyParty/reorderAll', { code: model }).subscribe(data => {
          this.toastr.success('Re Order เรียบร้อย', 'แจ้งเตือนระบบ', { timeOut: 2000 });
          setTimeout(() => {
            window.location.reload()
          }, 2000)
  ...
  deleteAll() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        ...
        this.serviceProviderService.post('policyParty/delete', { code: model }).subscribe(data => {
          this.toastr.success('ลบรายการเรียบร้อย', 'แจ้งเตือนระบบ', { timeOut: 2000 });
          setTimeout(() => {
            window.location.reload()
          }, 2000)
  ...
  exportAsXLSX(): void {
    ...
    this.serviceProviderService.post('policyParty/report/read', obj).subscribe(data => {
      let model: any = {};
      let result: any = [];
      model = data;
