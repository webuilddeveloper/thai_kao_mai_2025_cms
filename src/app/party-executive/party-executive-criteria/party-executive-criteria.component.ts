...
@Component({
  selector: 'app-party-executive-criteria',
  templateUrl: './party-executive-criteria.component.html',
  styleUrls: ['./party-executive-criteria.component.css']
})
export class PartyExecutiveCriteriaComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('partyExecutivePage') != null) {
      let model: any = [];
      model = JSON.parse(localStorage.getItem('partyExecutivePage'));
  ...
  readSequence() {
    this.criteriaModel.permission = this.permission;
    this.serviceProviderService.post('groupby/read', { permission: this.permission, title: 'partyExecutive' }).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  readCategory() {
    this.criteriaModel.permission = this.permission;
    this.serviceProviderService.post('partyExecutive/category/read', this.criteriaModel).subscribe(data => {
      let model: any = {};
      model = data;
