...
@Component({
  ...
  styleUrls: ['./party-executive.component.css']
})
export class PartyExecutiveComponent implements OnInit {
  model: any = [];
  listModel: string = '';
  ...
  ngOnInit(): void {
    ...
    this.serviceProviderService.SendIPAddress("ข่าวประชาสัมพันธ์");
  ...
  read() {
    ...
    this.serviceProviderService.post('partyExecutive/read', this.criteriaModel).subscribe(data => {
      setTimeout(() => {
        let model: any = {};
        model = data;
