...
@Component({
  selector: 'app-donate-category-list',
  templateUrl: './donate-category-list.component.html',
  styleUrls: ['./donate-category-list.component.css']
})
export class DonateCategoryListComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('donateCategoryPage') != null) {
      this.permission = JSON.parse(localStorage.getItem('donateCategoryPage'));
    }
  }
  ...
  create() {
    this.router.navigate(['donate-category-edit', ''], { skipLocationChange: true });
  }

  view(param) {
    this.router.navigate(['donate-category-edit', param], { skipLocationChange: true });
  }
  ...
  delete(param, idx) {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        this.serviceProviderService.post('donate/category/delete', { code: param.code }).subscribe(data => {
  ...
  deleteAll() {
    ...
    dialogRef.afterClosed().subscribe(result => {
      ...
      if (result) {
        ...
        this.serviceProviderService.post('donate/category/delete', { code: model }).subscribe(data => {
          this.toastr.success('ลบรายการเรียบร้อย', 'แจ้งเตือนระบบ', { timeOut: 2000 });
          setTimeout(() => {
