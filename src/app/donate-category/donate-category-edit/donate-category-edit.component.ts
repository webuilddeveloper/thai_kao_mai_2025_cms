...
@Component({
  selector: 'app-donate-category-edit',
  templateUrl: './donate-category-edit.component.html',
  styleUrls: ['./donate-category-edit.component.css']
})
export class DonateCategoryEditComponent implements OnInit {

  editModel: any = {};
  code: any;
  title = 'แก้ไขข้อมูลหมวดบริจาค';
  category: any;
  ...
  ngOnInit(): void {
    ...
    this.activetedRoute.queryParams.subscribe(params => {
      ...
      if (this.code != '') {
        this.title = 'แก้ไขข้อมูลหมวดบริจาค';
        this.read();
      }
    });
  ...
  create() {
    ...
    this.serviceProviderService.post('donate/category/create', this.editModel).subscribe(data => {
      let model: any = {};
      model = data;
      if (model.status === 'S') {
  ...
  read() {
    this.serviceProviderService.post('donate/category/read', { code: this.code, permission: 'all' }).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  update() {
    ...
    if (this.editModel.image != undefined)
      this.editModel.imageUrl = this.editModel.image[0].imageUrl;
    this.serviceProviderService.post('donate/category/update', this.editModel).subscribe(data => {
      let model: any = {};
      model = data;
      if (model.status === 'S') {
        /** spinner ends after 5 seconds */
  ...
  back() {
    this.router.navigate(['donate-category'], { skipLocationChange: true });
  }
