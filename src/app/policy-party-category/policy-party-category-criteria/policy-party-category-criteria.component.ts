...
@Component({
  selector: 'app-policy-party-category-criteria',
  templateUrl: './policy-party-category-criteria.component.html',
  styleUrls: ['./policy-party-category-criteria.component.css']
})
export class PolicyPartyCategoryCriteriaComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('policyPartyPage') != null) {
      let model: any = [];
      model = JSON.parse(localStorage.getItem('policyPartyPage'));
  ...
  readSequence() {
    this.criteriaModel.permission = this.permission;
    this.serviceProviderService.post('groupby/read', { permission: this.permission, title: 'policyParty' }).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  readCategory() {
    this.criteriaModel.permission = this.permission;
    this.serviceProviderService.post('policyParty/category/read', this.criteriaModel).subscribe(data => {
      let model: any = {};
      model = data;
