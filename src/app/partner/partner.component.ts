...
export class PartnerComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('partnerPage') != null) {
      let model: any = [];
      model = JSON.parse(localStorage.getItem('partnerPage'));
  ...
  read() {
    ...
    this.serviceProviderService.post('partner/read', this.criteriaModel).subscribe(data => {
      setTimeout(() => {
        let model: any = {};
        model = data;
