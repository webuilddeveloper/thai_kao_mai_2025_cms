...
@Component({
  selector: 'app-donate-criteria',
  templateUrl: './donate-criteria.component.html',
  styleUrls: ['./donate-criteria.component.css']
})
export class DonateCriteriaComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    if (localStorage.getItem('donatePage') != null) {
      let model: any = [];
      model = JSON.parse(localStorage.getItem('donatePage'));
  ...
  readSequence() {
    this.criteriaModel.permission = this.permission;
    this.serviceProviderService.post('groupby/read', { permission: this.permission, title: 'donate' }).subscribe(data => {
      let model: any = {};
      model = data;
  ...
  readCategory() {
    this.criteriaModel.permission = this.permission;
    this.serviceProviderService.post('donate/category/read', this.criteriaModel).subscribe(data => {
      let model: any = {};
      model = data;
